...
def test_update_bot_settings_retries_without_missing_strategy_mode_schema_column():
    from app.database.repositories import update_bot_settings

    table = MagicMock()
    table.eq.return_value = table
    table.execute.side_effect = [
        Exception("{'message': \"Could not find the 'strategy_mode' column of 'bot_settings' in the schema cache\", 'code': 'PGRST204'}"),
        MagicMock(data=[{"id": "row-1", "mode": "AUTO_DEMO"}]),
    ]
    table.update.return_value = table
    ...
    with patch("app.database.repositories.get_supabase", return_value=supabase):
        with patch("app.database.repositories.get_bot_settings", return_value={"id": "row-1"}):
            result = update_bot_settings({"strategy_mode": "AI_ONLY", "mode": "AUTO_DEMO"})

    assert table.update.call_args_list[0].args[0] == {"strategy_mode": "AI_ONLY", "mode": "AUTO_DEMO"}
    assert table.update.call_args_list[1].args[0] == {"mode": "AUTO_DEMO"}
    assert result["mode"] == "AUTO_DEMO"
